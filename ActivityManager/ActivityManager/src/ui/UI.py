...
class UI:
    ...
    def search_persons_and_activities(self):
        ...
        if option2 == "1":
            ...
            telephones = self.__person_services.search_by_phone_number(phone_number)
            for telephone in telephones:
                print(telephone)
        elif option2 == "2":
            name = input("Insert the name: ")
            persons = self.__person_services.search_by_name(name)
    ...
    def manage_people_and_activities(self):
        ...
        try:
            ...
            if option2 == "1":
                ...
                self.__activity_services.remove_all_occurrences(id)
            ...
            # ACTIVITY
                ...
                options = self.__person_services.get_list()
                if not id.isnumeric():
                    raise UIError("The ID is not numeric!")
                formatted_date = 0
                ...
                if not old_id.isnumeric() or not new_id.isnumeric():
                    raise UIError("The ID is not numeric!")
                activity=self.__activity_services.search_activity_based_on_id(old_id)
                try:
                    self.__activity_services.update_activity(old_id, new_id, new_list, new_formatted_date,
                                                             new_time, new_description, activity, options)
                except ValueError:
                    print("Invalidity!")
